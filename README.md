...
either an animated terminal UI or a fully scriptable CLI to drive it.
...
<!--
  docs/demo/dashboard.gif is generated by .github/workflows/demo.yml from
  tapes/*.tape on every push that touches tapes/**. None of the three
  GIFs referenced in this README (dashboard.gif here, up.gif and
  model-picker.gif further down) exist in the repo until that workflow
  runs at least once — until then they render as broken images. See
  CONTRIBUTING.md if you're regenerating them locally.
-->
...
- **Two front ends, one engine.** A full animated TUI for interactive use, and a `--json`/exit-code
  scriptable CLI for everything else — including for other agents driving AgentRoute itself.
